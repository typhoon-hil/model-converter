...
@pytest.mark.parametrize("Vsin3P, f, iRA_expected",
                         [(220, 50, 31.1127)])
@pytest.mark.parametrize("convert_to_tse, create_intermediate_file", doubled_parameter_values, indirect=True)
@pytest.mark.parametrize("load_and_compile", [use_vhil], indirect=True)
def test_r3(load_and_compile, Vsin3P, f, iRA_expected):

    # Set source value
    hil.set_source_sine_waveform(name='Vsin3P', rms=Vsin3P, frequency=f)

    # Start capture
    start_capture(duration=0.2, signals=['iRA'], executeAt=0)
    ...
    # Tests
    sig.assert_is_constant(iRA, during=(0.005 - 0.000001, 0.005 + 0.000001), at_value=around(iRA_expected, tol_p=0.01))
