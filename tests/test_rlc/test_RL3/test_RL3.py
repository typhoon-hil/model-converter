...
@pytest.mark.parametrize("Vsin3P, f, iRLA_expected",
                         [(220, 50, 29.68)])
@pytest.mark.parametrize("convert_to_tse, create_intermediate_file", doubled_parameter_values, indirect=True)
@pytest.mark.parametrize("load_and_compile", [use_vhil], indirect=True)
def test_rl3(load_and_compile, Vsin3P, f, iRLA_expected):

    # Set source value
    hil.set_source_sine_waveform(name='Vsin3P', rms=Vsin3P, frequency=f)

    # Start capture
    start_capture(duration=0.2, signals=['iRLA'], executeAt=0)
    ...
    # Tests
    sig.assert_is_constant(iRLA, during=(0.006 - 0.000001, 0.006 + 0.000001), at_value=around(iRLA_expected, tol_p=0.01))
    sig.assert_is_constant(iRLA, during=(0.0609689 - 0.000001, 0.0609689 + 0.000001), at_value=(-0.02,0.02))
