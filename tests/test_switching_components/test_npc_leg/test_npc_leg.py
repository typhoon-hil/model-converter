...
@pytest.mark.parametrize("VDC1, iR1_expected",
                         [(100, 10)])
@pytest.mark.parametrize("convert_to_tse, create_intermediate_file", doubled_parameter_values, indirect=True)
@pytest.mark.parametrize("load_and_compile", [use_vhil], indirect=True)
def test_npc_leg(load_and_compile, VDC1, iR1_expected):
    ...
    for switch in range(1,5):
        ...
        hil.set_pe_switching_block_control_mode(blockName='SwitchingLeg',
                                                  switchName="S" + str(switch),
                                                  swControl=True)
        hil.set_pe_switching_block_software_value(blockName='SwitchingLeg',
                                                  switchName="S" + str(switch),
                                                  value=1)
    ...
    # Tests
    sig.assert_is_constant(iR1, during=(0.015 - 0.0001, 0.015 + 0.0001), at_value=around(iR1_expected, tol_p=0.01))
