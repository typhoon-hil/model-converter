...
@pytest.mark.parametrize("Vsin3P, f, iRCA_expected",
                         [(220, 50, 0.1956)])
@pytest.mark.parametrize("convert_to_tse, create_intermediate_file", doubled_parameter_values, indirect=True)
@pytest.mark.parametrize("load_and_compile", [use_vhil], indirect=True)
def test_rc3(load_and_compile, Vsin3P, f, iRCA_expected):

    # Set source value
    hil.set_source_sine_waveform(name='Vsin3P', rms=Vsin3P, frequency=f)

    # Start capture
    start_capture(duration=0.2, signals=['iRCA'], executeAt=0)
    ...
    # Tests
    sig.assert_is_constant(iRCA, during=(0.02 - 0.000001, 0.02 + 0.000001), at_value=around(iRCA_expected, tol_p=0.02))
    sig.assert_is_constant(iRCA, during=(0.01502 - 0.000001, 0.01502 + 0.000001), at_value=(-0.02,0.02))
