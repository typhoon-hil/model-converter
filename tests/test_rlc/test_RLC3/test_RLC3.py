...
@pytest.mark.parametrize("Vsin3P, f, iRLCA_expected",
                         [(220, 50, 10.2389)])
@pytest.mark.parametrize("convert_to_tse, create_intermediate_file", doubled_parameter_values, indirect=True)
...
def test_rlc3(load_and_compile, Vsin3P, f, iRLCA_expected):
    ...
    # Tests
    sig.assert_is_constant(iRLCA, during=(0.02107 - 0.000001, 0.02107 + 0.000001), at_value=around(iRLCA_expected, tol_p=0.01))
    sig.assert_is_constant(iRLCA, during=(0.0160685 - 0.000001, 0.0160685 + 0.000001), at_value=(-0.02,0.02))
