...
@pytest.mark.parametrize("Vsin3P, f, iA_expected",
                         [(220, 50, 60.54)])
@pytest.mark.parametrize("convert_to_tse, create_intermediate_file", doubled_parameter_values, indirect=True)
@pytest.mark.parametrize("load_and_compile", [use_vhil], indirect=True)
def test_3ph_ac_cable(load_and_compile, Vsin3P, f, iA_expected):

    # Set source value
    hil.set_source_sine_waveform(name='Vsin3P', rms=Vsin3P, frequency=f)

    # Start capture
    start_capture(duration=0.2, signals=['iA'], executeAt=0)
    ...
    # Tests
    sig.assert_is_constant(iA, during=(0.00574 - 0.000001, 0.00574 + 0.000001), at_value=around(iA_expected, tol_p=0.01))
    sig.assert_is_constant(iA, during=(0.0107437 - 0.000001, 0.0107437 + 0.000001), at_value=(-0.02,0.02))
