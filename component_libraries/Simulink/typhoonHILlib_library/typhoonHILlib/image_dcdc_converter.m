...
if strcmp([num2str(sizex) ',' num2str(sizey)], old_size)
    ...
    convtype_txt = get_param(gcb,'ConvType');
    symm = get_param(gcb,'Symmetrical');
    y_delta = 0;

    if strcmp(symm, 'on')
        comp_string1 = ['Symmetrical'];
    else
        comp_string1 = [''];
        y_delta = 0.03;
    end
    comp_string2 = [convtype_txt ' Converter'];

    if strcmp(convtype_txt,'Select the')
        comp_string1 = '';
        y_delta = 0.03;
    end
    ...
    if sizex>100&sizey>150
        ...
        image('images/DC-DC.png',[(1-im_x)/2 (1.3-im_y)/2 im_x im_y]);
