...
if strcmp([num2str(sizex) ',' num2str(sizey)], old_size)&tog_draw
    ...
    rectype_txt = get_param(gcb,'RecType');
    if strcmp('on', three_phase)
        comp_string1 = ['Three-Phase'];
        comp_string2 = [rectype_txt ' Rectifier'];
    else
        comp_string1 = ['Single-Phase'];
        comp_string2 = [rectype_txt ' Rectifier'];
    end

    if strcmp(rectype_txt,'Select the')
        comp_string1 = '';
        y_delta = 0.03;
    end
    ...
    if sizex>100&sizey>150
        ...
        image('images/AC-DC.png',[(1-im_x)/2 (1.3-im_y)/2 im_x im_y]);
