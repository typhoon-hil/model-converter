...
@pytest.mark.parametrize("Vsin3P, f, s3p_closed, iR1_expected",
                         [(220, 50, True, 31.1127),
                          (220, 50, False, 0)])
@pytest.mark.parametrize("convert_to_tse, create_intermediate_file", doubled_parameter_values, indirect=True)
@pytest.mark.parametrize("load_and_compile", [use_vhil], indirect=True)
def test_bi_directional_switch(load_and_compile, Vsin3P, f, s3p_closed, iR1_expected):
    ...
    # Tests
    # Peak
    sig.assert_is_constant(iR1, during=(0.00500298 - 0.0001, 0.005 + 0.0001), at_value=around(iR1_expected, tol_p=0.01))
    # Min
    sig.assert_is_constant(iR1, during=(0.015 - 0.000001, 0.015 + 0.000001), at_value=around(-iR1_expected, tol_p=0.01))
    #Zero
    sig.assert_is_constant(iR1, during=(0.01 - 0.000001, 0.01 + 0.000001), at_value=(-0.01, 0.01 ))
